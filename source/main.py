...
from httpx import get, post, request
...
ORGANIZACAO = getenv('ORGANIZACAO')
PROJETO = getenv('PROJETO')
TOKEN = getenv('TOKEN')


class Azure:
    def __init__(self, organization, project, token: str):
    ...
    def __endpoint_item(self, item_type: str) -> str:
        """
        ...
        """
        return f"https://dev.azure.com/{self._organization}/{self._project}/_apis/wit/workitems/${item_type}?api-version=6.0" 
    
    def __endpoint_iterations(self, squad: str) -> str:
        """
        ...
        """
        return f'https://dev.azure.com/{self._organization}/{self._project}/{squad}/_apis/work/teamsettings/iterations?api-version=6.0'

    def __endpoint_tags(self) -> str:
        """
        ...
        """
        return f'https://dev.azure.com/{self._organization}/{self._project}/_apis/wit/tags?api-version=6.0-preview.1'

    def __get(self, url: str) -> request:
        """
            ...
            :type url: str
        """
        return get(url=url, headers=self._headers)

    def __processing_interation(self, response: dict) -> List[dict]:
        """
    ...
    def tags(self) -> List[str]:
        ...
        response = self.__get(endpoint).json()
        tags = [item['name'] for item in response['value']]
        pprint(tags)
        return tags
    ...
    descricao: str = '', pai: int = None, filho: int = None, modelo_contratacao: str = None, horas_consumidas: float = 0, horas_contratadas: float = 0) -> bool:
        """
            Cria um item de trabalho num projeto de template e campos personalizados, \
                suportando apenas os tipos: `Projeto`, `Produto`, `Entrega`, `Atividade`.
        """
    ...
    @staticmethod
    def __convert_utc(item: str) -> str:
        """
